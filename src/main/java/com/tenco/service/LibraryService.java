...
public class LibraryService {
    ...
    // 4. 학생 등록
    // 1. 이름과 학번이 비어 있는지 검사
    // 2. 통과하면 DAO 에 INSERT 처리를 위임 한다.
    //    유니크 걸려있는 student_id 는 DB에서 확인해야 함므로 여기서는 먼저 중복 검사를 안할 예정.
    ...
    // 도서 대출
    // 1. 도서 ID와 학생 ID가 1 이상인지 검사 ( AUTO_INCREMENT 는 1 부터 시작)
    ...
    // 4. 일치하면 비밀번호를 지운 Admin 객체를 반환, 아니면 null View 객체에 돌려 준다.
    public Admin authenticateAdmin(String adminId, String password) throws SQLException {
        ...
        // 2
        Admin admin = adminDAO.findMyAdminId(adminId);
